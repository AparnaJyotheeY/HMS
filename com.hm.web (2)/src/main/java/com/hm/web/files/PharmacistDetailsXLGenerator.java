...
public class PharmacistDetailsXLGenerator  extends AbstractExcelView {

	@Override
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workBook, HttpServletRequest req,
			HttpServletResponse resp) throws Exception {
	
		List<DoctorBean> nurse = (List<DoctorBean>) model.get("pharmacist");
		
		// create a new Excel sheet
        HSSFSheet sheet = workBook.createSheet("Pharmacist List");
        ...
        header.createCell(0).setCellValue("Pharmacist Id");
        ...
        for (DoctorBean dlist : nurse) {
        
            HSSFRow aRow = sheet.createRow(rowCount++);
           
            aRow.createCell(0).setCellValue(dlist.getPharmacistid());
